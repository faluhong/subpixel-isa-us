"""
    post-processing of the ISP time series by combining the IS change and NDVI threshold rule
    Also include the surface modification types in the outputs

    Two major steps
    (1) If IS expansion case, no post-processing is needed.
    ...
    For the running time, when using 110 cores, it takes about 24 hours
"""
...
from Basic_tools.datetime_datenum_convert import datenum_to_datetime_matlabversion
from evaluation.utils_evaluation import convert_8_tile_names_to_6_tile_names, convert_6_tile_names_to_8_tile_names

from post_processing.isp_ts_post_processing import (define_logger,
...
from analysis.utils_cal_index import (ndvi_cal)
from post_processing.utils_apply_permanent_natural_surface_mask import (apply_permanent_natural_surface_us_land_boundary_mask)
from analysis.isp_change_type_img import (output_is_change_type, add_pyramids_color_in_is_change_type_tif)
...
def binary_is_ndvi_based_post_processing_with_sm(array_valid_segment_mask_ori,
    ...
    Returns:
        array_valid_segment_mask_update: the updated mask to indicate which years contain valid COLD segment for the post-processing
        array_seamless_segment_mask_update: the updated seamless mask for the post-processing
    ...
    if len(list_segment_id_seamless) == 1:
        ...
        for i_seg in range(1, len(list_segment_id_seamless)):
            ...
            if (flag_natural_previous | flag_natural_current) & (surface_modification_flag == 'binary_is_ndvi'):
                # If any one of the previous or current segment is natural surface, then no NDVI post-processing is needed
                # It contains two types: IS expansion and IS reversal
                # no NDVI post-processing is needed
                pass
            elif (flag_natural_previous ) & (surface_modification_flag == 'is_expansion_ndvi'):
                # if previous segment is natural surface, then no NDVI post-processing is needed
                # Even though the current segment is natural surface, NDVI post-processing is needed
                # The post-processing will be applied to IS reversal types, but not IS expansion types
                pass
            else:
...
def pipeline_post_processing_isp_ts_with_sm_labels(img_stack_ts_is_pct,
    ...
    """
    pipeline for post-processing the ISP time series by considering the IS category change and the NDVI threshold

    More specifically:
        (1) Apply the NDVI threshold to IS intensification and decline types
        (2) No post-processing on the IS expansion types
        (3) The post-processing on the IS reversal types depends on the "surface_modification_flag" flag
            if  "surface_modification_flag" is "binary_is_ndvi", no post-processing on the IS reversal types, use binary_is_ndvi_based_post_processing function
...
# if __name__ =='__main__':
    ...
    vi_post_processing_rule = 'is_expansion_ndvi015_sm'
    threshold = 0.15
    surface_modification_flag = 'is_expansion_ndvi'
    ...
    # list_tile_running = ['h021v007', 'h030v006']  # for test
    
    # df_summary_high_res_isp = pd.read_excel(join(rootpath, 'data', 'ISP_from_high_res_lc', 'summary_high_resolution_isp.xlsx'))
    # array_tiles, array_tiles_counts = np.unique(df_summary_high_res_isp['tile_id'].values, return_counts=True)  # the ARD tiles and the image counts

    # for i_tile in range(0, len(array_tiles)):
    #     tile_name = convert_6_tile_names_to_8_tile_names(array_tiles[i_tile])
    #     print(tile_name)
    ...
    for i in range(0, each_core_block):
        ...
        if new_rank > len(list_tile_running) - 1:  # means that all folder has been processed
            ...
            logger.info(f'tile {tile_name} post-processing surface modification flag: {surface_modification_flag}')
            ...
            # save the post-processing IS change types images
