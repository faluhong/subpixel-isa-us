...
def read_cold_variable(predictor_variable_folder, tile_name, year, central_reflectance_flag='change', rootpath_project_folder=None):
    """
    read the export COLD variable .tif images for model training.
    The COLD variable .tif image comes from surface_reflectance_calculate_annual_bytile.py file
    ...
    :param tile_name:
    :param year:
    :param central_reflectance_flag:  # flag to use the stable (one value for one COLD segment) or annual change central reflectance, 'stable' or 'change', default as 'change'
...
def add_pyramids_color_in_is_change_type_tif(filename_tif, list_overview=None, colors=None):
    """
        add pyramids and color table in the IS change type tif file
    """
    ...
    if colors is None:
        ...
        colors = np.array([np.array([108, 169, 102, 255]) / 255,  # stable natural
                           ...
                           np.array([130, 202, 252, 255]) / 255,  # Surface modification
                           ])

    # Set the color for each value in the color table
    for i in range(1, len(colors) + 1):

        color_tuple = colors[i- 1] * 255
        color_tuple = color_tuple.astype(int)
