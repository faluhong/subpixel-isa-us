"""
    ...
    (2) If the segment is impervious surface, there are several ways to represent the whole segment.
       ...
       The final implementation uses the mean of the segment to represent the whole segment.
...
from pythoncode.model_training.utils_deep_learning import (get_proj_info, add_pyramids_color_in_nlcd_isp_tif)
from pythoncode.model_training.unet_prediction import predict_isp_output
...
def define_logger(filename_logger):
    """
        define the logger for CONUS isp post_processing
    """
...
def generate_post_processing_mask(array_t_start, array_t_end, mask_match, array_year):
    """
    generate the post_processing mask for the ISP time series
...
def post_processing_time_series(array_is_pct, array_is_mask, array_valid_segment_mask, array_seamless_segment_mask,
                                post_processing_rule='mean'):
    """
    ...
    :param post_processing_rule: rule to apply the post-processing to the segment, default is mean
    ...
    for i_seg in range(0, len(list_segment_id)):
        ...
        if post_processing_rule == 'max':
            ...
            raise ValueError('error in the post_processing flag')
...
def pipeline_post_processing_isp_ts(img_stack_ts_is_pct,
                                    ...
                                    logger):
    """
    pipeline for post_processing the ISP time series
...
# if __name__ =='__main__':
    ...
    for i in range(0, each_core_block):
        ...
        if new_rank > len(list_tile_running) - 1:  # means that all folder has been processed
            ...
            # f.write(f'{tile_name} ISP post_processing running finished')
            # f.close()
