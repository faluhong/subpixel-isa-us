...
from Basic_tools.datetime_datenum_convert import datetime_to_datenum_matlabversion
from Basic_tools.mat_to_dataframe import mat_to_dataframe

from cold_reccg_process.surface_reflectance_calculate_annual_bytile import (mask_flag_generate,
                                                                            get_annual_overall_reflectance)

from deep_learning_isp.unet_prediction import (unet_model_load,
                                               mosaic_prediction,
                                               predict_isp_output)
...
# if __name__ == "__main__":
    ...
    # list_year = np.arange(2000, 2002, 1)
    # list_year = np.array([1985, 1990, 1995, 2005, 2010, 2015, 2020])
    # list_year = np.array([2011, 2012, 2013, 2014, 2016, 2017, 2018, 2019, 2021])
    # list_year = np.array([2001, 2002, 2004, 2006, 2007, 2008, 2009])
    # list_year = np.array([2003])
    ...
    ##
    list_tile_name = os.listdir(path_cold_reccg)
    list_tile_name = [name for name in list_tile_name if os.path.isdir(join(path_cold_reccg, name))]    # keep the folder only
    list_tile_name.sort()
